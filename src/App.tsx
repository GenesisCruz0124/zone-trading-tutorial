...
function App() {
  ...
  const { nav, examples } = getContent(language)

  return (
    <div className="min-h-screen bg-tv-bg">
      <nav className="sticky top-0 z-10 flex items-center gap-2 overflow-x-auto border-b border-tv-border bg-tv-bg/95 px-4 py-2 backdrop-blur sm:px-8">
          ...
          <div className="flex flex-1 gap-1 overflow-x-auto">
            {nav.map((link) => (
              <a
                key={link.id}
                href={`#${link.id}`}
                className="whitespace-nowrap rounded px-3 py-1.5 text-sm text-fg-muted hover:bg-tv-panel hover:text-tv-teal"
              >
                {link.label}
              </a>
            ))}
